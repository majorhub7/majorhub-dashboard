...
const App: React.FC = () => {
    ...
    // Horizontal scroll support for mouse wheel
    useEffect(() => {
        const handleWheel = (e: WheelEvent) => {
            // Only handle if we have a significant vertical scroll
            if (e.deltaY !== 0) {
                e.preventDefault();
    ...
    if (loading) {
        return (
            <div className="h-screen w-screen flex items-center justify-center bg-background-light dark:bg-slate-950">
                ...
                <>
                    {selectedProject && (
                        <ProjectDetailsModal
                            ...
                            onReorderGoals={async (projectId, goals) => {
                                try {
                                    if (reorderGoals) {
                                        await reorderGoals(projectId, goals);
                                    }
